    ...
    def declarations(self):
        ...
        while True:
            if self.current_token.type == VAR:
                ...
                self.eat(SEMI)
                block_node = self.block()
                proc_node = ProcedureDecl(proc_name, params, block_node)
                declarations.append(proc_node)
                self.eat(SEMI)
            else:
                break
...
class SemanticAnalyzer(NodVisiter):
    def __init__(self):
...
class SourceToSourceCompiler(NodVisiter):
    def __init__(self):
        self.current_scope = None
        self.output = None
    ...
    def visit_BinOp(self, node):
        ...
        result_str = '%s %s %s' % (left_str, node.op.value, right_str)
        return result_str
    ...
    def visit_VarDecl(self, node):
        ...
        result_str = '   var %s%s : %s;' % (var_name, self.current_scope.scope_level, type_name)
    ...
    def visit_Assign(self, node):
        ...
        # left-hand side
        left_str = self.visit(node.left)
        result_str = '%s := %s;' % (left_str, right_str)
        return result_str
    ...
    def visit_Var(self, node):
        ...
        result_str = '<%s%s : %s>' % (var_name, self.current_scope.scope_level, var_symbol.type.name)
        return result_str
    ...
    def visit_ProcedureDecl(self, node):
        ...
        for param in node.params:
            ...
            formal_params.append(
                '%s%s : %s' % (param_name, self.current_scope.scope_level, param_type.name)
            )
